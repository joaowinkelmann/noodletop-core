...
export class Rand {
     ...
     * @param length The length of the ID string. Default is 8. (Collision probability is 1 in 62^length)
    ...
    static id(length: number = 8, includeTimestamp: boolean = true): string {
        ...
        if (includeTimestamp) {
            ...
            result = length.toString(36) + timestampString + result; // Store the length at the beginning of the id
        }
        return result;
    }
    ...
    static dateFromId(id: string): Date {
        const length = parseInt(id.charAt(0), 36); // Retrieve the length from the id // TODO: This is kinda dumb, perhaps we could calculate the length based on the id length
        let timestampString = id.slice(1, 9); // gets the first 8 characters from the id, after the length
